...
public class MouvementScript : MonoBehaviour {
	...
	void FixedUpdate()
	{
		...
		Debug.Log (mouveHorizontal);
		Debug.Log (mouveVertical);

		transform.Rotate(0,mouveHorizontal*OptionSnake.movementSpeed,0);
		charac.SimpleMove(transform.TransformDirection(Vector3.forward) * OptionSnake.movementSpeed * mouveVertical);

		// end add

		if (controller == null)
		{
			Debug.Log ("Controler is null");
			return;
		}
